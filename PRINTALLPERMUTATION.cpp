...
void printPermutaion (int ind , string s , set <string> &st) {
	if ( ind == s.size()) {
		st.insert(s);
		return ;
	}
	...
	for ( int i = 0 ; i < s.size() ; i++) {
		swap( s.at(ind) , s.at(i));  // fist step to swap
		printPermutaion( ind + 1 , s , st); // second step is to again call the function with indext + 1
...
void solve() {
	...
	// this program will generate all possilbe permutation of the given string
	// here i m using recursion with backtrack and set to maintiain the unique string
	// the approach used here is swapping the two characters at position i and j at a time

	set <string> st;
	// function ( int index , string , ds to store)
	printPermutaion( 0 , s , st);
